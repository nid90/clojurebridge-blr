...
(+ 1 2)
#+END_SRC
- All program code is written as s-expressions, or parenthesized lists, and all s-expressions evaluate to a value.
...
- As you might have noticed an expression looks like a list of some kind - (1, 2, 3) is a valid s-expression.
- LISP originally stood for LISt Processing, and lists are still at the core of the language.
- In fact, they form the most basic way to compose expressions, or sentences.
- A list is a single expression which has multiple parts.
- For instance, this list - (1 2 3) - contains three elements: the numbers 1, 2, and 3.a
- Most of the "code" that you will write wll be in the form of lists
- We will cover more about lists when we cover collection types in Clojure.
