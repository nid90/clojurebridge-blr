...
- Infix vs. prefix notation [TODO]
  ...
  - Infix notation  
    - Infix notation places the operator *in* between the operands
      ...
      - The order of operations is not obvious, deciding which operation happens first 
        requires deciding on an operator precedence hierarchy.
  - Postfix notation
    - Postfix notation places the operator before or *pre* the operands
      eg: + 1 + 2  * 3 4
          + 1 + 2  12
          + 1 14
          15
 ...
 Clojure uses the prefix notation, and pairs it with the s-expressions we've already seen to give us a concise and unambiguous notation.
 eg: (+ 1 2 (* 3 4))
     (+ 1 2 12)
     15
