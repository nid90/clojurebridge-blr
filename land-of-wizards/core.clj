...
(comment "To describe all the objects at a given location, the `objects-at-location` function isfirst called to get a collection (in this case a vector) of all the objects at the location, and then invoke the function stored in `object-whereabouts` on all elements of the collection by using `map`. `map` returns a lazy sequence of elements (which in this case are strings). The strings in this sequence are the `join`ed to form a single string.")
